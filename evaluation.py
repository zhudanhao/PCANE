...
def evaluate_ROC_other(X_test, Embeddings,dim):
    y_true = [ X_test[i][2] for i in range(len(X_test))]
    y_predict = []
    for i in range(len(X_test)):
        if X_test[i][0] not in Embeddings:
            vec1 = np.random.randn(dim)
        else:
            vec1 = Embeddings[X_test[i][0]]
        if X_test[i][1] not in Embeddings:
            vec2 = np.random.randn(dim)
        else:
            vec2 = Embeddings[X_test[i][1]]
        y_predict.append(cosine_similarity(vec1,vec2))
    roc = roc_auc_score(y_true, y_predict)
    if roc < 0.5:
        roc = 1 - roc
    return roc
